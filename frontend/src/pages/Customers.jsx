...
const inputStyle = {
  ...
  fontSize: '0.9rem', fontFamily: 'inherit', outline: 'none', width: '100%', transition: 'border-color 0.2s',
};
    ...
    <div className="card" style={{ marginBottom: 'var(--space-xl)' }}>
      ...
      <form onSubmit={handleSubmit} style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: 'var(--space-md)' }}>
        <div>
          ...
          <input style={inputStyle} name="name" type="text" placeholder="e.g. Jane Smith"
            value={form.name} onChange={handleChange} required minLength={1} maxLength={255} disabled={submitting} />
        </div>
        <div>
          <label style={labelStyle}>Email Address *</label>
          <input style={inputStyle} name="email" type="email" placeholder="jane@example.com"
        ...
        <div>
          ...
          <input style={inputStyle} name="phone" type="tel" placeholder="+1 555 000 1234"
            value={form.phone} onChange={handleChange} maxLength={20} disabled={submitting} />
        </div>
...
export default function Customers() {
  ...
  const avatarColor = (id) => {
    const colors = ['#4f46e5', '#0284c7', '#16a34a', '#d97706', '#ec4899', '#a78bfa', '#34d399', '#fb923c'];
  ...
  return (
    <div>
      ...
      {showForm && <CustomerForm onSubmit={handleCreate} onCancel={closeForm} submitting={submitting} error={formError} />}

      {loading ? (
        <div className="state-container"><div className="spinner" /><span>Loading customers…</span></div>
      ) : customers.length === 0 ? (
        <div className="state-container">
          <div className="state-icon">👥</div>
          <div className="state-title">No customers yet</div>
          <div className="state-sub">Click "Add Customer" to register your first customer.</div>
        </div>
      ) : (
        <div className="card" style={{ padding: 0 }}>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th>Registered</th><th style={{ textAlign: 'right' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {customers.map((c) => (
                  <tr key={c.id}>
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                        <div style={{ width: 36, height: 36, borderRadius: '50%', background: avatarColor(c.id), display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '0.75rem', fontWeight: 700, color: '#fff', flexShrink: 0, userSelect: 'none' }}>
                          {initials(c.name)}
                        </div>
                        <div>
                          <div style={{ fontWeight: 500 }}>{c.name}</div>
                          <div style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>#{c.id}</div>
                        </div>
                      </div>
                    </td>
                    <td><a href={`mailto:${c.email}`} style={{ color: 'var(--color-info)' }}>{c.email}</a></td>
                    <td className="td-muted">
                      {c.phone ? <a href={`tel:${c.phone}`} style={{ color: 'var(--color-text-muted)' }}>{c.phone}</a> : '—'}
                    </td>
                    <td className="td-muted">
                      {new Date(c.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'short', day: 'numeric' })}
                    </td>
                    <td style={{ textAlign: 'right' }}>
                      <button className="btn btn-danger" style={{ padding: '5px 14px', fontSize: '0.8rem' }} onClick={() => handleDelete(c)}>
                        🗑 Delete
                      </button>
                    </td>
                  </tr>
          ...
          <div style={{ padding: 'var(--space-sm) var(--space-lg)', borderTop: '1px solid var(--color-border)', fontSize: '0.8rem', color: 'var(--color-text-muted)', textAlign: 'right' }}>
